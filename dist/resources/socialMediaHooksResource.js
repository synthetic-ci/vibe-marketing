export function createSocialMediaHooksResource(hooksData) {
    return {
        uri: "vibe://hooks/social-media-hooks",
        metadata: {
            title: "Social Media Hooks",
            description: "Comprehensive collection of hooks for various social media platforms organized by network and category",
        ...
        handler: async (uri) => {
            ...
            for (const [networkKey, network] of Object.entries(hooksData.networks)) {
                ...
                for (const [categoryKey, category] of Object.entries(network.categories)) {
            ...
            yamlContent += "\ncategories:\n";
            yamlContent += "  global:\n";
            for (const [categoryKey, category] of Object.entries(hooksData.categories.global)) {
                yamlContent += `    ${categoryKey}:\n`;
                yamlContent += `      description: "${category.description}"\n`;
                yamlContent += `      keywords: [${category.keywords.map((k) => `"${k}"`).join(", ")}]\n`;
            }
            // Add usage tips
            yamlContent += "\nusage_tips:\n";
