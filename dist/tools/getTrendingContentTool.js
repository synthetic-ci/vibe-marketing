import { z } from "zod";
import { validateHyperFeedApiKey } from "../utils/apiValidation.js";
...
export const getTrendingContentToolDefinition = {
    ...
    inputSchema: {
        networks: z
            ...
            .optional()
            .default("twitter")
            .describe("Social media networks to get trending content from (e.g., 'twitter' and 'reddit')"),
        limit: z
            .number()
            .optional()
...
export const getTrendingContentHandler = (trendingContentService, trendingContentFormatter, apiKey) => async ({ networks = "twitter", limit, }) => {
    try {
        ...
        const responseText = trendingContentFormatter.formatTrendingResponse(data);
        return {
            content: [
                {
                    type: "text",
...
export const createValidatedTrendingContentHandler = (trendingContentService, trendingContentFormatter, apiKey) => {
    return async (request) => {
        console.log('get-trending-content tool called with request:', request);
        if (!apiKey) {
            console.log('No HyperFeed API key provided');
            throw new Error('HyperFeed API key is required to use the get-trending-content tool. ' +
                'Please create an API key at https://app.hyperfeed.ai and update your Smithery profile with the newly created API key.');
        }
        console.log('Validating API key...');
        console.log('Validating API key (token length:', apiKey.length, 'characters)');
