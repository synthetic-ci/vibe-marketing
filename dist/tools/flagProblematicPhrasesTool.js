...
export const flagProblematicPhrasesHandler = (doNotUseData) => async (args) => {
    try {
        const { text } = args;
        ...
        const responseText = `🚩 **Found ${foundPhrases.length} problematic phrase(s) in the text:**\n\n` +
            foundPhrases
                .map((item, index) => `${index + 1}. "${item.phrase}" (at position ${item.position})`)
                .join("\n") +
            "\n\n**Suggestions:**\n" +
