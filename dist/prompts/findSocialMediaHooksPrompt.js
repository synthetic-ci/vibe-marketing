...
import { z } from "zod";
export function findSocialMediaHooksPrompt(hooksData) {
    return {
        name: "find-social-media-hooks",
        definition: {
            title: "Find Social Media Hooks",
            description: "Guide for finding effective social media hooks by network and category",
            argsSchema: {
                network: completable(z.string(), (value = "") => {
                    // Get available networks from hooksData
                    const networks = Object.keys(hooksData.networks);
                    return networks.filter((n) => n.toLowerCase().startsWith(value.toLowerCase()));
                }),
                category: completable(z.string(), (value = "", context) => {
                    ...
                    if (selectedNetwork && hooksData.networks[selectedNetwork]) {
                        const networkCategories = Object.keys(hooksData.networks[selectedNetwork].categories);
                        // Merge and deduplicate categories
                        categories = [...new Set([...categories, ...networkCategories])];
                    }
                    // Filter based on user input
                    return categories.filter((c) => c.toLowerCase().startsWith(value.toLowerCase()));
                }),
                context: z.string(),
            },
        },
        handler: ({ network, category, context, }) => ({
            messages: [
                {
                    role: "user",
                    content: {
