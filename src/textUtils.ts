 ...
 */
export function getTweetCharacterLength(text: string): number {
	...
	const emojiPattern =
		/[\u{1F000}-\u{1F9FF}]|[\u{1F000}-\u{1F9FF}][\u200D\uFE0F\u{1F000}-\u{1F9FF}]*/gu;
 ...
 * - Most other characters count as 1 character
 */
export function getMetaCharacterLength(text: string): number {
...
export function validateTwitterBeforeFoldCharacterLimit(text: string): boolean {
...
export function validateMetaBeforeFoldCharacterLimit(text: string): boolean {
...
export function validateTiktokBeforeFoldCharacterLimit(text: string): boolean {
...
export function validateLinkedinBeforeFoldCharacterLimit(
	text: string,
): boolean {
	...
	if (lines.length > 3) {
		console.error(
			`Content exceeds the line limit before fold, ${lines.length} lines (max 3)`,
		);
		return false;
	}
...
export function validateYoutubeTitleBeforeFoldCharacterLimit(
	text: string,
): boolean {
	const length = getMetaCharacterLength(text); // YouTube uses the same character counting rules as Meta
	if (length > 70) {
		console.error(
			`YouTube title exceeds the character limit before fold, ${length} characters (max 70)`,
		);
		return false;
	}
	return true;
...
export function validateYoutubeDescriptionBeforeFoldCharacterLimit(
	text: string,
): boolean {
	const length = getMetaCharacterLength(text); // YouTube uses the same character counting rules as Meta
	if (length > 157) {
		console.error(
			`YouTube description exceeds the character limit before fold, ${length} characters (max 157)`,
		);
		return false;
	}
	return true;
...
/**
 * Social media platform types for before-fold truncation
 */
 ...
 */
function truncateForGeneric(text: string, limit: number): string {
 ...
 */
function truncateForLinkedIn(text: string): string {
